...
function Home() {
  ...
  return (
   ...
   {/* ======================== HERO ======================== */}
{/*
  FIX: replaced h-[80svh] with the `.hero-viewport` class (defined in
  styles.css) which declares `height: 80vh` first and `height: 80svh`
  second. Safari versions that don't understand svh simply ignore the
...
<section className="hero-viewport relative w-full overflow-hidden ink-section">
  ...
  <div className="absolute inset-0 z-10 h-full flex flex-col justify-end pb-2">
    <div className="mx-auto w-full max-w-[1500px] px-5 md:px-10">
      ...
      {/*
        FIX: bg-[oklch(0.65_0.25_27)] hard-baked a raw oklch() value
        into the compiled stylesheet with no fallback. Swapping to
        bg-[var(--ember)] points at the CSS variable defined in
        :root, which DOES get a hex fallback for pre-15.4 Safari via
        the @supports block in styles.css.
      */}
      <div className="hero-fade flex items-center gap-3 text-grotesk text-xs tracking-[0.4em] text-white/70 mb-6">
      ...
      <h1 className="text-display text-[18vw] md:text-[10vw] leading-[0.85] text-white tracking-[-0.04em]">
        ...
        <span className="block overflow-hidden">
          <span className="split-line block text-[var(--ember)] italic">
            RITUAL.
          </span>
        </span>
      </h1>
      ...
      <div className="mt-10 mb-8 flex flex-col md:flex-row md:items-center gap-6 hero-fade">
        ...
        <Link
          to="/about"
          className="text-white/80 hover:text-white text-grotesk tracking-[0.3em] underline underline-offset-8 decoration-1 decoration-[var(--accent)]"
        >
          READ THE MANIFESTO
        </Link>
      </div>
      ...
      <section className="bone-section py-24">
        <div className="mx-auto max-w-[1500px] px-5 md:px-10">
          <div className="flex items-end justify-between mb-10 reveal">
            <div>
              <p className="text-sm tracking-[0.35em] uppercase text-[var(--accent)]">
                — Featured Collection
              </p>
              <h2
                className="mt-3 text-[60px] md:text-[100px] leading-[0.9] tracking-[-0.05em]"
          ...
          <div className="flex justify-center mt-16 reveal">
            <Link
              to="/shop"
              className="group inline-flex items-center gap-3 px-10 py-4 rounded-full bg-black text-white tracking-[0.25em] text-sm uppercase hover:bg-[var(--accent)] transition duration-300"
            >
              View More
              <FiArrowRight className="transition-transform duration-300 group-hover:translate-x-1" />
            </Link>
      ...
      <section className="bone-section py-24">
        <div className="mx-auto max-w-[1500px] px-5 md:px-10">
          <div className="text-center reveal">
            <p className="text-grotesk text-xs tracking-[0.4em] text-[var(--accent)]">
              — THE WEARERS
            </p>
            <h2 className="mt-3 text-display text-5xl md:text-7xl">
              Worn by the world.
          ...
          <div className="mt-16 grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="reveal glass-light p-7 rounded-sm">
                ...
                <div className="mt-6 text-grotesk text-xs tracking-[0.3em] text-[var(--muted-foreground)]">
                  {t.name} — {t.city}
                </div>
              </div>
            ))}
