...
//   function Checkout() {
//     const { user, cart = [], cartTotal = 0, clearCart, discount = 0 } = useApp();
...
//       if (!nameRegex.test(firstName)) return toast.error("Invalid first name");
...
//         await axios.post(`${API}/api/orders`, pendingOrder);
...
//   // ✅ Memoized so these heavy lookups don't re-run on every keystroke/render.
//   // Unmemoized country-state-city calls were the leading suspect for the
//   // iOS Safari blank-screen / memory-crash symptom.
//   const countries = useMemo(() => {
//     console.log("[Checkout] computing countries list");
//     try {
//       const result = Country.getAllCountries();
//       console.log("[Checkout] countries computed OK, count =", result?.length);
//       return result;
//     } catch (err) {
...
//     console.log("[Checkout] computing cities list for", { country, stateCode });
//     try {
//       const result = City.getCitiesOfState(country, stateCode);
//       console.log("[Checkout] cities computed OK, count =", result?.length);
//       return result;
//     } catch (err) {
//       console.error("[Checkout] City.getCitiesOfState() THREW", err);
//       throw err;
//     }
//   }, [country, stateCode]);
